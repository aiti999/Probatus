...
export type NavbarProps = {
  navigationItems: { name: string; minWidth: number; link: string }[]; // Add 'link' property to navigationItems
  className?: string;
};
    ...
    <header className={`fixed top-0 left-1 right-12 py-5 px-12 z-50 bg-opacity-85 flex items-center justify-between ${className}`}>
      ...
      <nav className="flex mq450:hidden flex-row items-center pr-12 justify-end gap-10 text-center font-semibold text-gray-300">
        {navigationItems.map((item) => (
          <a
            key={item.name}
            href={item.link} // Link to the section
      ...
      <button className="ss:hidden text-white relative left-16 bg-transparent" onClick={toggleSidebar} aria-label="Toggle menu">

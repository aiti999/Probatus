...
const MobileSidebar: React.FC<MobileSidebarProps> = ({ isSidebarOpen, toggleSidebar, navigationItems }) => (
    ...
    <div className="flex flex-col h-full p-4">
      {/* Close button */}
      <button className="self-end text-white absolute top-4 right-4 bg-transparent" onClick={toggleSidebar} aria-label="Close menu">
      ...
      <nav className="flex flex-col items-center justify-center mt-24 gap-6 text-center font-semibold text-gray-300">
        {navigationItems.map((item) => (
          <a key={item.name} href={item.link} className="relative leading-5 hover:text-white text-white font-semibold inline-block no-underline" onClick={toggleSidebar}>
            {item.name}
          </a>
        ))}
      </nav>
      ...
      <div className="mt-auto flex justify-center h-10 pb-8">
        <BookButton />
      </div>
    </div>
  </div>
